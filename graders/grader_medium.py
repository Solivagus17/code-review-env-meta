...
def grade(action: Action, state: dict, step: int) -> Reward:
    ...
    breakdown.verdict_accuracy = 0.25 if action.verdict == gt['verdict'] else 0.0
    ...
    for issue in issues:
        for lc in action.line_comments:
            if (abs(lc.line_number - issue['line']) <= 3 and
                ...
                break
    breakdown.bug_detection = (found_weight / max(total_weight, 0.01)) * 0.35
    ...
    breakdown.comment_quality = wc_score + kw_score

    # 4. Fix suggestions (0.10): at least 1 concrete fix required
    breakdown.efficiency_bonus = min(len(action.suggested_fixes) * 0.05, 0.10)
    ...
    total = max(0.001, min(0.999, sum(vars(breakdown).values())))
